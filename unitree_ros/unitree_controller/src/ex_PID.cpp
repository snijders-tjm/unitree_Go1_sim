...
// Added to set the robot to lie on its back-------------------------------------------------------------
    ros::Publisher ex_Tommy = n.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 1000);
...
// -------------------------------------------------------------------------------------------------------
    ...
    timePoseData << "t_ms;t_ros_secs;FR_calfq_ref;FR_calfq;FRcalfqraw \n";
    bool firstcycle = true;

    while (ros::ok()){
        if (firstcycle) {
            model_state_pub.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(M_PI, 0, 0);
            ex_Tommy.publish(model_state_pub);
            firstcycle = false;
        }

        if (time_ms>= 5000){
            lowCmd.motorCmd[2].q = -2.3;
        }
        double secs = ros::Time::now().toSec();

        timePoseData << time_ms << ";" << secs << ";" << lowCmd.motorCmd[2].q << ";" << lowState.motorState[2].q << ";" << lowState.motorState[2].q_raw << "\n";
        time_ms += 1;
        sendServoCmd();
