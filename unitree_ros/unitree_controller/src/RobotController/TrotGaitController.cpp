...
#define MAX_X_VELOCITY 0.035    // maximal x velocity
#define MAX_Y_VELOCITY 0.02     // maximal y velocity
#define MAX_YAW_RATE 0.5        // maximal yaw rate around the z-axis
#define Z_LEG_LIFT 0.4         // swing leg lift
...
void TrotGaitController::updateStateCommand(const sensor_msgs::Joy::ConstPtr& msg,
    ...
    command.velocity[0] = msg->axes[1] * max_x_velocity; // left joystick up down
    command.velocity[1] = msg->axes[0] * max_y_velocity; // left joystick left right
    command.yaw_rate = msg->axes[3] * max_yaw_rate; // right joystick left right
    ...
    if(use_button){
        ...
        } else if(msg->buttons[6]){ // back button
            auto_rest = !auto_rest;    
            if(!auto_rest)
                trot_needed = true;
            use_button = false;
...
///////////////////////////////////////////////////////////////////////////////
void TrotGaitController::updateVelCommand(const geometry_msgs::Twist::ConstPtr& msg,
                State& state, Command& command) {
    command.velocity[0] = msg->linear.x * max_x_velocity;
    command.velocity[1] = msg->linear.y * max_y_velocity;
    command.yaw_rate = msg->angular.z * max_yaw_rate;
...
Eigen::Matrix<float, 3, 4> TrotGaitController::step(State& state,
    ...
    else
    {
        ...
        for(int i = 0; i < 4; i++)
            new_foot_locations(2,i) = command.robot_height*0.9;
        return new_foot_locations;
    }
}
///////////////////////////////////////////////////////////////////////////////
...
Eigen::Matrix<float, 3, 4> TrotGaitController::run(State& state, Command& command)
{
    ...
    state.robot_height = command.robot_height*0.9;
