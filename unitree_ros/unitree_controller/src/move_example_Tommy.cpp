...
int main(int argc, char **argv)
{
    ...
    ros::Publisher move_publisher = n.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 1000);
    ...
    ros::ServiceClient robot_client = n.serviceClient<gazebo_msgs::GetModelState>("/gazebo/get_model_state");

    gazebo_msgs::GetModelState robot_state;
    geometry_msgs::Pose robot_pose, pose_ref, pose_error;
    ...
    const double k_factor = -0.001; //error dynamics factor

    motion_init();

    ofstream timePoseData;
    timePoseData.open("MoveExampleTommy_" + robot_name + "_gazebo.csv");
    timePoseData << "t,refP_x,refP_y,refP_z,refO_x,refO_y,refO_z,refO_w,P_x,P_y,P_z,O_x,O_y,O_z,O_w,\n";
    robot_state.request.model_name = robot_name + "_gazebo";

    while (ros::ok()){
        if (robot_client.call(robot_state)){
            robot_pose = robot_state.response.pose;
//            cout<<"The [x,y,z] position of the robot is: ["<<robot_pose.position.x<<","<<robot_pose.position.y<<","<<robot_pose.position.z<<"] \n";
        }

        pose_ref.position.x = radius * sin(2*M_PI*(double)time_ms/period);
        pose_ref.position.y = radius * cos(2*M_PI*(double)time_ms/period);
        pose_ref.position.z = 0.5;
        pose_ref.orientation = tf::createQuaternionMsgFromRollPitchYaw(0, 0, - 2*M_PI*(double)time_ms/period);

        pose_error.position.x = robot_pose.position.x - pose_ref.position.x;
        pose_error.position.y = robot_pose.position.y - pose_ref.position.y;
        pose_error.position.z = robot_pose.position.z - pose_ref.position.z;

        pose_error.orientation.x = robot_pose.orientation.x - pose_ref.orientation.x;
        pose_error.orientation.y = robot_pose.orientation.y - pose_ref.orientation.y;
        pose_error.orientation.z = robot_pose.orientation.z - pose_ref.orientation.z;
        pose_error.orientation.w = robot_pose.orientation.w - pose_ref.orientation.w;

        model_state_pub.pose.position.x = robot_pose.position.x + k_factor*pose_error.position.x;
        model_state_pub.pose.position.y = robot_pose.position.y + k_factor*pose_error.position.y;
        model_state_pub.pose.position.z = robot_pose.position.z + k_factor*pose_error.position.z;

        model_state_pub.pose.orientation.x = robot_pose.orientation.x + k_factor*pose_error.orientation.x;
        model_state_pub.pose.orientation.y = robot_pose.orientation.y + k_factor*pose_error.orientation.y;
        model_state_pub.pose.orientation.z = robot_pose.orientation.z + k_factor*pose_error.orientation.z;
        model_state_pub.pose.orientation.w = robot_pose.orientation.w + k_factor*pose_error.orientation.w;

        /*
        control logic
        */
        timePoseData << time_ms << ";" << pose_ref.position.x << ";" << pose_ref.position.y << ";" << pose_ref.position.z << ";" << pose_ref.orientation.x << ";" << pose_ref.orientation.y << ";" << pose_ref.orientation.z << ";" << pose_ref.orientation.w << ";" << robot_pose.position.x << ";" << robot_pose.position.y << ";" << robot_pose.position.z << ";" << robot_pose.orientation.x << ";" << robot_pose.orientation.y << ";" << robot_pose.orientation.z << ";" << robot_pose.orientation.w << ",\n";
